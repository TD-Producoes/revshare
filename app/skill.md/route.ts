...
function buildMarkdown(): string {
  ...
  const bodyLines = [
    ...
    "```bash",
    `curl -X POST ${CANONICAL_ORIGIN}/api/revclaw/intents \\\n  -H "Authorization: Bearer $AGENT_SECRET" \\\n  -H "X-RevClaw-User-Id: $USER_ID" \\\n  -H "Content-Type: application/json" \\\n  -d '{"kind":"REWARD_CREATE","payload_json":{"project_id":"...","project_name":"...","name":"First 10 sales","milestoneType":"COMPLETED_SALES","milestoneValue":10,"rewardType":"DISCOUNT_COUPON","rewardPercentOff":25,"fulfillmentType":"AUTO_COUPON","earnLimit":"ONCE_PER_MARKETER","availabilityType":"UNLIMITED","visibility":"PUBLIC"},"idempotency_key":"..."}'`,
    "```",
    "2) Human opens `approval_url` and approves (magic-link approval works without Telegram).",
    "3) Bot executes creation via RevClaw:",
    "```bash",
    `curl -X POST ${CANONICAL_ORIGIN}/api/revclaw/projects/:id/rewards \\\n  -H "Authorization: Bearer $ACCESS_TOKEN" \\\n  -H "X-RevClaw-Intent-Id: $INTENT_ID" \\\n  -H "Content-Type: application/json" \\\n  -d '{"project_name":"...","name":"First 10 sales","milestoneType":"COMPLETED_SALES","milestoneValue":10,"rewardType":"DISCOUNT_COUPON","rewardPercentOff":25,"fulfillmentType":"AUTO_COUPON","earnLimit":"ONCE_PER_MARKETER","availabilityType":"UNLIMITED","visibility":"PUBLIC"}'`,
    "```",
    "",
    "Notes:",
