...
function buildMarkdown(): string {
  ...
  const bodyLines = [
    ...
    "Create an intent:",
    "```bash",
    `curl -X POST ${CANONICAL_ORIGIN}/api/revclaw/intents \\\n  -H "Authorization: Bearer $ACCESS_TOKEN" \\\n  -H "Content-Type: application/json" \\\n  -d '{"kind":"PROJECT_PUBLISH","payload_json":{"project_id":"..."},"idempotency_key":"..."}'`,
    "```",
    "",
    "Then the human approves/denies via Telegram/web. Once approved, execute the protected action providing the intent (mechanism depends on endpoint; see `/revclaw/auth.md` for the current contract).",
    "",
    "---",
    "",
    "### Identity + install",
