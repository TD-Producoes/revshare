...
function buildAuthMarkdown(): string {
  ...
  const frontmatterLines = [
    ...
    "name: revclaw-auth",
    `version: ${appVersion}`,
    "description: Authentication flow for RevClaw agents (bot notifies human on any channel; human approves via web claim page).",
    `homepage: ${CANONICAL_ORIGIN}`,
    `metadata: {\"revclaw\":{\"api_base\":\"${CANONICAL_ORIGIN}/api\",\"registration_endpoint\":\"${CANONICAL_ORIGIN}/api/revclaw/agents/register\",\"claim_flow\":\"bot-sends-claim-url\",\"claim_page\":\"${CANONICAL_ORIGIN}/revclaw/claim\",\"identity_verification_endpoint\":\"${CANONICAL_ORIGIN}/api/revclaw/agents/verify-identity\",\"claim_status_endpoint\":\"${CANONICAL_ORIGIN}/api/revclaw/agents/:agent_id/claim-status\",\"token_exchange_endpoint\":\"${CANONICAL_ORIGIN}/api/revclaw/tokens\",\"token_refresh_endpoint\":\"${CANONICAL_ORIGIN}/api/revclaw/tokens/refresh\",\"auth_scheme\":\"bearer-access-token\"}}`,
    "---",
  ];
  ...
  const bodyLines = [
    ...
    "3. The bot sends the human a claim link (any channel) → human approves in browser.",
    ...
    "",
    "### 3) Human claim / approval (web)",
    "After registration, the bot must send the human the `claim_url` returned by the register endpoint.",
    "",
    "- The human opens the link in a browser and approves after logging into RevShare.",
    "- The backend does **not** send Telegram/WhatsApp messages; the bot handles that on whatever channel it has.",
    "",
    "Claim page:",
    `- ${CANONICAL_ORIGIN}/revclaw/claim?claim_id=...`,
    "",
    "(For implementers: keep claim ids short-lived and single-use.)",
    "",
