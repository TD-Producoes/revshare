...
/**
 * GET /api/revclaw/projects/:id/attribution-link
 *
 * Returns the marketer's attribution short link for a project.
...
export async function GET(
  ...
  try {
    ...
    const existing = await prisma.attributionShortLink.findUnique({
      where: { projectId_marketerId: { projectId, marketerId: agent.userId } },
      select: { code: true },
    });

    const record = existing ?? (await createUniqueShortLink(projectId, agent.userId));
    const origin = new URL(request.url).origin;
    const url = `${origin}/a/${record.code}`;

    return NextResponse.json({ data: { code: record.code, url } }, { status: 200 });
  } catch (err) {
